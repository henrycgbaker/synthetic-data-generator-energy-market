...
class TestBuildSchedules:
    ...
    def test_build_schedule_with_linear_growth(self):
        ...
        variables = {
            'cap.wind': {
                'regimes': [
                    {'name': 'growing', 'dist': {'kind': 'linear', 'start': 5000.0, 'slope': 10.0}}
                ]
            }
        }
    ...
    def test_build_all_required_variables(self):
        ...
        variables = {
            ...
            'cap.nuclear': {'regimes': [{'name': 's', 'dist': {'kind': 'const', 'v': 6000.0}}]},
            'cap.coal': {'regimes': [{'name': 's', 'dist': {'kind': 'const', 'v': 8000.0}}]},
            'cap.gas': {'regimes': [{'name': 's', 'dist': {'kind': 'const', 'v': 12000.0}}]},
            'cap.wind': {'regimes': [{'name': 's', 'dist': {'kind': 'const', 'v': 7000.0}}]},
            'cap.solar': {'regimes': [{'name': 's', 'dist': {'kind': 'const', 'v': 5000.0}}]},
            'avail.nuclear': {'regimes': [{'name': 's', 'dist': {'kind': 'const', 'v': 0.95}}]},
            'avail.coal': {'regimes': [{'name': 's', 'dist': {'kind': 'const', 'v': 0.90}}]},
            'avail.gas': {'regimes': [{'name': 's', 'dist': {'kind': 'const', 'v': 0.95}}]},
            'eta_lb.coal': {'regimes': [{'name': 's', 'dist': {'kind': 'const', 'v': 0.33}}]},
            'eta_ub.coal': {'regimes': [{'name': 's', 'dist': {'kind': 'const', 'v': 0.38}}]},
            'eta_lb.gas': {'regimes': [{'name': 's', 'dist': {'kind': 'const', 'v': 0.48}}]},
            'eta_ub.gas': {'regimes': [{'name': 's', 'dist': {'kind': 'const', 'v': 0.55}}]},
            'bid.nuclear.min': {'regimes': [{'name': 's', 'dist': {'kind': 'const', 'v': -200.0}}]},
            'bid.nuclear.max': {'regimes': [{'name': 's', 'dist': {'kind': 'const', 'v': -50.0}}]},
            'bid.wind.min': {'regimes': [{'name': 's', 'dist': {'kind': 'const', 'v': -200.0}}]},
            'bid.wind.max': {'regimes': [{'name': 's', 'dist': {'kind': 'const', 'v': -50.0}}]},
            'bid.solar.min': {'regimes': [{'name': 's', 'dist': {'kind': 'const', 'v': -200.0}}]},
            'bid.solar.max': {'regimes': [{'name': 's', 'dist': {'kind': 'const', 'v': -50.0}}]},
        }
...
class TestRegimeScheduleValueAt:
    ...
    def test_value_at_const_distribution(self):
        ...
        schedule = RegimeSchedule(
            ...
            freq='h',
            segments=[{'name': 'const', 'days': 1, 'dist': {'kind': 'const', 'v': 100.0}, 'transition_hours': 0}],
            rng=None,
            series_map={}
        )
    ...
    def test_value_at_clamped_to_index(self):
        ...
        schedule = RegimeSchedule(
            ...
            start_ts=pd.Timestamp('2024-01-01'),
            freq='h',
            segments=[{'name': 's', 'days': 1, 'dist': {'kind': 'const', 'v': 100.0}, 'transition_hours': 0}],
            rng=None,
            series_map={}
        )
