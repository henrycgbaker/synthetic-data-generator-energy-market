...
class TestLinearCapacityScenario:
    ...
    def test_coal_phaseout_full_integration(self, temp_output_dir):
        ...
        from synthetic_data_pkg.config import TopConfig, IOConfig
        ...
        config = TopConfig(
            ...
            variables={
                "fuel.gas": {"regimes": [{"name": "stable", "dist": {"kind": "const", "v": 30.0}}]},
                "fuel.coal": {"regimes": [{"name": "stable", "dist": {"kind": "const", "v": 25.0}}]},
                
                # LINEAR CAPACITY CHANGES
                "cap.coal": {"regimes": [{"name": "declining", "dist": {"kind": "linear", "start": 8000.0, "slope": -0.913}}]},  # 8000 -> 0 in 1 year
                "cap.gas": {"regimes": [{"name": "increasing", "dist": {"kind": "linear", "start": 12000.0, "slope": 0.685}}]},  # 12000 -> 18000 in 1 year
                "cap.wind": {"regimes": [{"name": "building", "dist": {"kind": "linear", "start": 7000.0, "slope": 0.571}}]},    # 7000 -> 12000 in 1 year
                "cap.solar": {"regimes": [{"name": "building", "dist": {"kind": "linear", "start": 5000.0, "slope": 0.571}}]},   # 5000 -> 10000 in 1 year
                
                # CONSTANT CAPACITIES
                "cap.nuclear": {"regimes": [{"name": "constant", "dist": {"kind": "const", "v": 6000.0}}]},
        ...
        for idx in sample_indices:
            ...
            print(f"{hour:6d} {day:4.0f} {row['cap.coal']:8.1f} {row['cap.gas']:8.1f} {row['cap.wind']:8.1f} {row['cap.solar']:8.1f}")
        
        # TEST 1: Coal should decline
        print(f"\nCoal: {coal_values[0]:.1f} -> {coal_values[-1]:.1f}")
        assert coal_values[0] > coal_values[-1], f"Coal should decline: {coal_values[0]} -> {coal_values[-1]}"
        assert coal_values[0] == pytest.approx(8000.0, abs=10.0), "Coal should start at 8000"
        
        # TEST 2: Gas should increase  
        print(f"Gas: {gas_values[0]:.1f} -> {gas_values[-1]:.1f}")
        assert gas_values[-1] > gas_values[0], f"Gas should increase: {gas_values[0]} -> {gas_values[-1]}"
        assert gas_values[0] == pytest.approx(12000.0, abs=10.0), "Gas should start at 12000"
        
        # TEST 3: Wind should increase
        print(f"Wind: {wind_values[0]:.1f} -> {wind_values[-1]:.1f}")
        assert wind_values[-1] > wind_values[0], f"Wind should increase: {wind_values[0]} -> {wind_values[-1]}"
        assert wind_values[0] == pytest.approx(7000.0, abs=10.0), "Wind should start at 7000"
        
        # TEST 4: Solar should increase
        print(f"Solar: {solar_values[0]:.1f} -> {solar_values[-1]:.1f}")
        assert solar_values[-1] > solar_values[0], f"Solar should increase: {solar_values[0]} -> {solar_values[-1]}"
        assert solar_values[0] == pytest.approx(5000.0, abs=10.0), "Solar should start at 5000"
        ...
        for i in range(0, len(df) - sample_freq, sample_freq):
            ...
            assert curr["cap.coal"] >= next_day["cap.coal"], f"Hour {i}: Coal should be declining"
            assert curr["cap.gas"] <= next_day["cap.gas"], f"Hour {i}: Gas should be increasing"
            assert curr["cap.wind"] <= next_day["cap.wind"], f"Hour {i}: Wind should be increasing"
            assert curr["cap.solar"] <= next_day["cap.solar"], f"Hour {i}: Solar should be increasing"
        ...
        print(f"Coal daily change std dev: {coal_diff_std:.2f} (should be small for linear)")
        assert coal_diff_std < 5.0, "Coal decline should be linear (low std dev in daily changes)"
        
        # Check gas increase is linear
        gas_series = df["cap.gas"].values[::24]
        gas_diffs = np.diff(gas_series)
        gas_diff_std = np.std(gas_diffs)
        print(f"Gas daily change std dev: {gas_diff_std:.2f} (should be small for linear)")
    ...
    def test_schedules_directly(self):
        ...
        schedules = build_schedules(
            ...
            supply_regime_planner={"mode": "local_only"},
            variables={
                "cap.coal": {"regimes": [{"name": "declining", "dist": {"kind": "linear", "start": 8000.0, "slope": -10.0}}]},
                "cap.gas": {"regimes": [{"name": "constant", "dist": {"kind": "const", "v": 12000.0}}]},
            },
            series_map={},
        )
        ...
        for i in range(1, len(values)):
            assert values[i] < values[i-1], f"Coal should decline: {values[i]} >= {values[i-1]}"
